...
<template>
    <div class='container'>
        ...
        <div v-if='!audition' class='panel-recorder' :class='{active: recording}'>
            <svg class="icon icon-play" @mouseenter='extraPrompt(true, "试听")' @mouseleave='extraPrompt(false)' @mouseup.stop='handleAudition' @mouseup.once.stop='getElements' viewBox="0 0 1024 1024" version="1.1" xmlns="http://www.w3.org/2000/svg" p-id="2815" xmlns:xlink="http://www.w3.org/1999/xlink"><path d="M204.454912 138.15808 820.340736 512.016384l-616.723456 373.819392L204.454912 138.15808M204.454912 84.583424c-29.586432 0-53.574656 23.988224-53.574656 53.574656l-0.837632 747.676672c-0.013312 19.47648 10.529792 37.421056 27.5456 46.877696 7.99232 4.368384 16.939008 6.670336 26.028032 6.695936 9.993216 0.014336 19.776512-2.771968 28.278784-8.04352L848.605184 557.559808c25.1392-15.591424 32.896-48.617472 17.317888-73.783296-4.369408-7.011328-10.30656-12.949504-17.317888-17.304576L232.733696 92.653568c-8.48896-5.25824-18.285568-8.04352-28.278784-8.04352l0 0L204.454912 84.583424z" p-id="2816"></path></svg>
        ...
        <div v-else class='panel-audition'>
            ...
            <div class='progress-wrapper'>
                ...
                <svg v-if='!playing' class='play'  @click='audio[playing ? "pause" : "play"]();' viewBox="0 0 1024 1024" version="1.1" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink"><path d="M268.7 79.1l586.1 373c33.6 21.4 43.5 65.9 22.1 99.4-5.7 8.9-13.2 16.4-22.1 22.1l-586.1 373c-33.6 21.4-78.1 11.5-99.4-22.1-7.4-11.6-11.3-25-11.3-38.7v-746c0-39.8 32.2-72 72-72 13.7 0 27.1 3.9 38.7 11.3z" p-id="2590"></path></svg>
                <svg v-else class='pause' @click='audio[playing ? "pause" : "play"]()' viewBox="0 0 100 100"><rect x='0' y='0' width='100' height='100' rx='10' ry='10'></rect></svg>
            </div>
            <!-- 确定是否发送录音 -->
            <div class='button-group'>
                <button class='send' @click='$emit("message", {data: [{type: "audio", content: recorder.wavBuffer}], duration: audio.duration}); quit();'>✔</button>
                <button class='delete' @click='quit'>✖</button>
            </div>
            <!-- 播放试听 -->
            <audio :src="recordURL" @durationchange='time = audio.duration' @playing='playing = true; update()' @pause='pause' style='display: none;' ref='audio'></audio>
        </div>
...
<script lang="ts">
    ...
    export default class Sound extends Vue {
        ...
        mounted() {
            ...
            new Recorder((time: number)=> time >= 15 ? (this.handleAudition(), alert('录音不得超过15s')) : (this.time = time))
        ...
        getElements() {
            setTimeout(()=> {
                ...
                this.progress.style.strokeDasharray = this.progress.style.strokeDashoffset = this.progressLen;
            }, 0);
        }
        ...
        update() {
            if(this.playing) {
                ...
                this.ID = setInterval(()=> {
                    this.time += 0.05;
                    this.progress.style.strokeDashoffset = (1 - this.time / this.audio.duration) * this.progressLen + '';
                    this.time >= this.audio.duration && this.audio.pause();
                }, 50);       
            }
        }
        ...
        pause() {
            ...
            this.progress.style.strokeDashoffset = this.progressLen + '';
            this.playing = false;
